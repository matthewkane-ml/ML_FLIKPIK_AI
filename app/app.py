...
def show_movie_cards(df, score_col=None, max_items=24):
    ...
    for row in df.to_dict("records"):
        ...
        # Use a pre-filtered poster URL when one already exists.
        # This avoids re-fetching TMDB posters for sections like Trending This Week.
...
def main():
    ...
    if page == "🏠 Home":
        ...
        if global_search:
            ...
            st.markdown("<div class='row-header'><h2>🔥 Trending This Week</h2><span class='view-all'>View all →</span></div>", unsafe_allow_html=True)
            trending = model.popularity_df.copy()

            # Remove duplicate movie titles before checking posters.
            ...
            # -------------------------------------------------
            # KEEP ONLY MOVIES WITH REAL WORKING TMDB POSTERS
            # -------------------------------------------------
            if TMDB_API_KEY:
                trending["poster_url"] = trending["title"].apply(fetch_tmdb_poster)
                trending = trending[trending["poster_url"].notna()]
                trending = trending[trending["poster_url"].apply(poster_url_works)]
            else:
                st.warning("TMDB_API_KEY is missing. Trending movies with real posters cannot be filtered yet.")
