...
def search_tmdb_movie(title):
    """Return the best TMDB movie match for a title."""
    if not TMDB_API_KEY:
        return None
...
@st.cache_data(show_spinner=False)
def get_tmdb_trailer(title):
    """Fetch the best official trailer from TMDB's movie videos endpoint."""
    ...
    url = TMDB_MOVIE_VIDEOS_URL.format(movie_id=movie_id)
    ...
    try:
        ...
        # Prefer official trailers, then trailers, then teasers/clips.
        preferred = sorted(
            youtube_videos,
            key=lambda video: (
                video.get("type") != "Trailer",
                not video.get("official", False),
                "trailer" not in video.get("name", "").lower(),
            ),
        )
        ...
        return {
            "source": "TMDB",
            "movie_id": movie_id,
            "movie_title": movie.get("title") or clean_movie_title(title),
            "title": video.get("name", "Official Trailer"),
            "channel": "TMDB Video",
            ...
            "thumbnail": f"https://img.youtube.com/vi/{video_key}/hqdefault.jpg",
        }
    except Exception:
        return None
...
@st.cache_data(show_spinner=False)
def get_best_trailer(title):
    """Use TMDB official videos first, then fall back to YouTube Data API search."""
    tmdb_trailer = get_tmdb_trailer(title)
    if tmdb_trailer:
        return tmdb_trailer

    youtube_trailer = get_youtube_trailer(title)
    if youtube_trailer:
        youtube_trailer["source"] = "YouTube Search"
    ...
    for row_start in range(0, len(filtered_records), cols_per_row):
        ...
        for col, row in zip(cols, row_items):
            ...
            with col:
                ...
                trailer = get_best_trailer(title)

                if trailer:
                    with st.expander("▶ Watch Trailer"):
                        st.video(trailer["url"])
                        st.markdown(f"**{trailer.get('title', 'Official Trailer')}**")
                        st.caption(f"Source: {trailer.get('source', 'TMDB')}")

                        if trailer.get("thumbnail"):
                            st.image(trailer["thumbnail"], use_container_width=True)
                else:
                    st.link_button(
                        "Search Trailer",
                        get_youtube_search_url(title),
...
def main():
    ...
    with st.sidebar:
        ...
        st.write(f"🎬 TMDB Trailers: **{'Connected' if TMDB_API_KEY else 'Missing'}**")
    ...
    if page == "🏠 Home":
        ...
        st.markdown("<div class='section-head'><div class='section-title'>🎭 Actor / Director Discovery</div><div class='section-link'>TMDB-powered idea zone</div></div>", unsafe_allow_html=True)
        st.info("This section is ready for your next upgrade: connect TMDB person search to discover movies by actor or director.")
        q = st.text_input("Search a movie title for now", placeholder="Example: Inception")
        results = search_engine.search(title_query=q, genre="All", min_year="", max_year="", limit=60)
        show_movie_cards(results, max_items=18) if display == "Cards" else show_movie_table(results)
        ...
        st.markdown("<div class='section-head'><div class='section-title'>▶️ Reviews & Trailers</div><div class='section-link'>TMDB official videos + YouTube fallback</div></div>", unsafe_allow_html=True)
        ...
        if len(movie_options) == 0:
            ...
            trailer = get_best_trailer(movie)

            st.markdown(f"### 🎬 {movie}")

            if trailer:
                source = trailer.get("source", "Trailer")
                official = "Official" if trailer.get("official") else "Best Match"
                st.caption(f"{source} • {official} • {trailer.get('title', 'Trailer')}")

                st.video(trailer["url"], autoplay=True, muted=True)
                st.markdown(f"[Open trailer on YouTube]({trailer['url']})")
            else:
                search_url = get_youtube_search_url(movie)

                if not TMDB_API_KEY:
                    st.info("Add a TMDB_API_KEY to fetch official TMDB trailers.")
                if not YOUTUBE_API_KEY:
                    st.info("Add a YOUTUBE_API_KEY to use YouTube search fallback.")

                st.warning("No official trailer was found automatically.")
                st.markdown(f"[🔎 Search YouTube for {movie}]({search_url})")

    elif page == "🤖 Ask FlikPik":
        st.markdown(
            "<div class='section-head'><div class='section-title'>🤖 Ask FlikPik AI</div><div class='section-link'>Conversational entertainment discovery</div></div>",
            unsafe_allow_html=True,
        )

        st.caption(
            "Talk naturally with FlikPik. Ask for genres, moods, actors, directors, decades, or movies with a similar vibe."
        )
            ...
            st.session_state.messages = [
                {
                    "role": "assistant",
                    "content": (
                        "🎬 Hey — I’m FlikPik AI. Tell me what you’re in the mood for. "
                        "You can ask for movies by vibe, genre, actor, director, "
                        "decade, streaming mood, or even say things like "
                        "'something mind-bending like Inception but newer.'"
                    ),
                }
            ]
        ...
        quick_prompts = [
            "Mind-bending sci-fi after 2010",
        ...
        for col, quick_prompt in zip(quick_cols, quick_prompts):
            with col:
                if st.button(
                    quick_prompt,
                    use_container_width=True,
                    key=f"quick_prompt_{quick_prompt}",
                ):
                    selected_quick_prompt = quick_prompt

        # ----------------------------------------------------------
        # USER INPUT
        # ----------------------------------------------------------
        user_prompt = st.chat_input(
            "Ask FlikPik for a movie vibe, actor, director, genre, or recommendation..."
        )

        if selected_quick_prompt:
            user_prompt = selected_quick_prompt
        ...
        if user_prompt:
            ...
            # ------------------------------------------------------
            previous_prompt = st.session_state.conversation_context.get("last_prompt", "")
            ...
            if is_followup and previous_prompt:
                enhanced_prompt = (
                    f"{previous_prompt}. Additional preference: {user_prompt}"
                )
            else:
            ...
            # ------------------------------------------------------
            # BUILD MORE CONVERSATIONAL RESPONSE
            # ------------------------------------------------------
            genres = parsed.get("genres", [])
            excluded = parsed.get("excluded_genres", [])
            year_filter = (
                parsed.get("year")
                or parsed.get("year_min")
                or parsed.get("decade")
            )

            assistant_intro = "Got it — "

            response_parts = []

            if genres:
                response_parts.append(
                    f"{', '.join(genres)} movies"
                )
            ...
            assistant_intro += f" Here are {len(recs)} recommendations you might enjoy."

            final_response = f"{assistant_intro}
...
{response}"
            ...
            st.session_state.conversation_context["last_prompt"] = enhanced_prompt
            ...
            with st.chat_message("assistant"):
                ...
                if parsed:
                    ...
                    metrics[0].metric(
                        "Genres",
                        ", ".join(genres) if genres else "Any",
                    )
                    ...
                    metrics[2].metric(
                        "Year Filter",
                        str(year_filter) if year_filter else "Any",
                    )
                ...
                if explanations:
                    with st.expander("🧠 Why FlikPik chose these"):
                        for explanation in explanations[:top_n]:
                ...
                followups = [
                    ...
                    "More action",
                    "Less horror",
                    "More like the first movie",
                ]

                for followup_col, followup in zip(followup_cols, followups):
                    with followup_col:
                        st.button(
                            followup,
                            use_container_width=True,
