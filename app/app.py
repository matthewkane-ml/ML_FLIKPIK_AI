...
def show_movie_cards(df, score_col=None, max_items=24):
    ...
    # ------------------------------------------------------
    # Only keep movies that have a real TMDB poster.
    # This prevents missing-poster movies from taking up blank space in the layout.
    filtered_records = []
    ...
    # ------------------------------------------------------
    # DISPLAY ONLY MOVIES WITH REAL POSTERS
    # ------------------------------------------------------
    for row_start in range(0, len(filtered_records), cols_per_row):
        row_items = filtered_records[row_start : row_start + cols_per_row]

        # Important: create only as many columns as there are valid poster cards.
        # This removes empty spaces when some movies were filtered out.
